...
public class DocumentService {
	...
	public Document get(String index, String type, String id) {
        GetResponse responseEs = client.prepareGet(index, type, id)
                .setFields(
                        FsRiverUtil.Doc.ATTACHMENT,
                        FsRiverUtil.Doc.META + "." + FsRiverUtil.Doc.Meta.TITLE,
                        FsRiverUtil.Doc.FILE + "." + FsRiverUtil.Doc.File.CONTENT_TYPE)
        ...
        String content_type = (String) responseEs.getField(FsRiverUtil.Doc.FILE + "." + FsRiverUtil.Doc.File.CONTENT_TYPE).getValue();
        BytesArray content = (BytesArray) responseEs.getField(FsRiverUtil.Doc.ATTACHMENT).getValue();

		response.setContent(new String(Base64.encode(content.toBytes())));
